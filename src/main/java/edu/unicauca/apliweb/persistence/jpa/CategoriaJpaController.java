    ...
    public void create(Categoria categoria) throws PreexistingEntityException, Exception {
        ...
        try {
            ...
            for (Producto productoListProducto : categoria.getProductoList()) {
                Categoria oldCodCategoriaOfProductoListProducto = productoListProducto.getCodCategoria();
                productoListProducto.setCodCategoria(categoria);
                productoListProducto = em.merge(productoListProducto);
                if (oldCodCategoriaOfProductoListProducto != null) {
                    oldCodCategoriaOfProductoListProducto.getProductoList().remove(productoListProducto);
                    oldCodCategoriaOfProductoListProducto = em.merge(oldCodCategoriaOfProductoListProducto);
                }
            }
        ...
        try {
            ...
            for (Producto productoListOldProducto : productoListOld) {
                if (!productoListNew.contains(productoListOldProducto)) {
                    productoListOldProducto.setCodCategoria(null);
                    productoListOldProducto = em.merge(productoListOldProducto);
                }
            }
            for (Producto productoListNewProducto : productoListNew) {
                if (!productoListOld.contains(productoListNewProducto)) {
                    Categoria oldCodCategoriaOfProductoListNewProducto = productoListNewProducto.getCodCategoria();
                    productoListNewProducto.setCodCategoria(categoria);
                    productoListNewProducto = em.merge(productoListNewProducto);
                    if (oldCodCategoriaOfProductoListNewProducto != null && !oldCodCategoriaOfProductoListNewProducto.equals(categoria)) {
                        oldCodCategoriaOfProductoListNewProducto.getProductoList().remove(productoListNewProducto);
                        oldCodCategoriaOfProductoListNewProducto = em.merge(oldCodCategoriaOfProductoListNewProducto);
                    }
                }
            }
            em.getTransaction().commit();
    ...
    public void destroy(Integer id) throws NonexistentEntityException {
        ...
        try {
            ...
            for (Producto productoListProducto : productoList) {
                productoListProducto.setCodCategoria(null);
                productoListProducto = em.merge(productoListProducto);
            }
            em.remove(categoria);
            em.getTransaction().commit();
